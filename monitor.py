...
def get_secret(key):
    ...
    try:
        ...
        raise ValueError(f"Secret '{key}' non trouvé dans env vars ou st.secrets")
...
def ask_ai_advanced(engine, question, url_cible):
    ...
    if engine == "perplexity":
        ...
        headers = {"Authorization": f"Bearer {get_secret('PERPLEXITY_API_KEY')}", "Content-Type": "application/json"}
        try:
